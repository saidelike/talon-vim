...
#   * vim_mode_visual.talon   - commands that only work in visual mode
...
###
# TODO: this needs to be fixed in terminal mode because we need to use the relative lines instead of the absolute ones
go row <number>:
...
(paste from | pastor) [register] <user.key_unmodified>:
    user.vim_run_any_motion_exterm('"{key_unmodified}p')
...
remove newlines from register <user.key_unmodified>:
    user.vim_run_command_exterm(":let @{key_unmodified}=substitute(strtrans(@{key_unmodified}),'\\^@',' ','g')\n")
